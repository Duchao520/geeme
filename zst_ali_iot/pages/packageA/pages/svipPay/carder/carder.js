...
Component({
  ...
  methods: {
    ...
    getCoupon(coupon){
      this.setData({
        priType: this.data.priType == 1 ? '' : this.data.priType,
        coupon: coupon,
        couponPayFlag: true
      });
      this.calMoney();
    ...
    init() {
      ...
      MAIN_CARD = STORE_ACTIVE.biSvipcard;
      MAIN_CARD.discountRate_ceil = accMul(Number(MAIN_CARD.discountRate ? MAIN_CARD.discountRate : 0),10);
      MAIN_CARD.money_ceil = accDiv(Number(MAIN_CARD.money),100);
     ...
     * */
      calMoney() {
        ...
        if(_self.data.coupon && Number(Subtr(Number(totalAmount), Number(_self.data.coupon.couponMoney))) <=0 && _self.data.couponPayFlag){   //实付金额小于卡券优惠
          ...
          COUPON_PAY.discount = accMul(Number(_self.data.totalAmount),100);
        }else{
          ...
          //卡主支付
          SVIP.actPay = Math.ceil(accMul(accMul(totalAmount,_self.data.MAIN_CARD.discountRate),100));
          SVIP.discount = accMul(Subtr(totalAmount,SVIP.actPay),100)
          //折扣满减
      ...
      priTypeEvent() {
        ...
        if (_self.data.priType == 1) {
          PAY_INFO.actPay = _self.data.SVIP.actPay;
      ...
      clickPayEvent(){
        ...
        switch (true){
          case (!Boolean(_self.data.priType) && _self.data.couponPayFlag && Boolean(_self.data.coupon)):
            activeType = 2;break;
          case (_self.data.priType == 1):
            if(_self.data.couponPayFlag && _self.data.coupon){
              _self.data.PAY_INFO.actPay_ceil == 0 ? activeType = 2 : activeType = 9;  //金额为0使用优惠券支付
            }else{
              activeType = 8;
            }
            break;
        ...
        _data = {
          ...
          orderType: activeType == 8 ? 5 : 1,
          mainCardId: (activeType == 8 || activeType == 9) ? _self.data.MAIN_CARD.id : '',
          activeNum: (activeType == 1 || activeType == 5) ? _self.data.INTERGRAL_ACT.activeNum : '',
          couponId: (activeType == 2 || activeType == 5 || activeType == 9) ? _self.data.coupon.id : '',
        ...
        app.API.placeorderCheck(_data).then(res => {
          if(res.state == 1){
            ...
            if(activeType == 8 || activeType == 9 || _self.data.PAY_INFO.actPay_ceil == 0){
              _self.pay();
            }else{
              var CASHIER_OBJ = {
                  appName: 'cashier',
      ...
      pay(r){
        ...
        var integralCount = (_self.data.coupon && _self.data.couponPayFlag) ? String(Subtr(accDiv(_self.data.PAY_INFO.discount, 100),_self.data.coupon.couponMoney)) :  String(accDiv(_self.data.PAY_INFO.discount, 100));
        _data = {
          paramsJsonStr: (activeType !== 8 && activeType!==9 && _self.data.PAY_INFO.actPay_ceil != 0 ) ? JSON.stringify(r) : '{}',
          userId: (activeType == 8 || activeType == 9 || _self.data.PAY_INFO.actPay_ceil == 0) ? _self.data.USERINFO.id : '',
          iotAppId: app.store.appId.getStore().data,
          totalAmount: _self.data.totalAmount,
          phone: _self.data.USERINFO.realPhone ? _self.data.USERINFO.realPhone : '',
          amount: _self.data.PAY_INFO.actPay_ceil,
