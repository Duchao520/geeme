<template>
  <div>
    ...
    <header class="bg-f " :style="'padding-top:'+status_bar_height+'px'">
      <h3 class="border-buttom">商家列表</h3>
      ...
      <div class="search-box bg-f" v-show="searchState">
        <form action="javascript:return true;">
          <input type="search bg-f" placeholder="请输入商家名称" @keyup.13="searchStaff" v-model="nameOrPhone"
                 ref="input1"/>
          <button v-if="" type="button" class="fz15" @click="hideSearch">取消</button>
        </form>
      </div>
    </header>
      ...
      <!--{{list.length}}-->
       <ul class="eve_mes" v-if="list.length!=0">
           <li v-for="(item,index) in list" :key="index">
             ...
             <div class="eve_ri" @click="goStoreList(item)">
                共{{item.shopStoreCount}}家门店 <i class="iconfont icon-youjiantou1"></i>
             </div>
           </li>
       </ul>
      <ul class="eve_mes" v-if="list.length==0">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goStoreList(item){
        this.$router.push({path: '/setStoreList',
            query: item
          },
        );
      },
      ...
      getAgentStaff() {
        ...
        let _data = {
          ...
          shopName: _self.nameOrPhone ? _self.nameOrPhone :'',
        };
        _self.$getSignIn(_data).then(sec => {
          getShopList(_data,sec).then(res => {
            let _data = res.data;
            if (_data.state == 1) {
