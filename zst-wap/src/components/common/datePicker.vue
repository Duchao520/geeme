<template>
  <transition name="alertMsg">
    <div class="datePicker" v-show="isShow" @click.stop="">
      <v-head :title="'自定义时间'"
              :replace="1"
              @replaceEvent="isShow = false"
              ref="vHead"></v-head>
      <div class="box_padding" style="padding: .52rem .34rem;">
        <div class="flex_box justify_space_between align_center" style="margin-bottom: .55rem">
          <span class="fz-34">选择时间</span>
          <span class="fz-30 fc-F7094D">注：时间范围小于等于365天</span>
        </div>
        <div class="flex_box justify_space_between align_center" style="margin-bottom: .44rem">
          ...
          <i class="fz-36">-</i>
          <div class="date-selector fz-30 fc-8D8E98" :class="{'active': endSelector}">
            <x-button type="default"
                      plain
                      @click.native="endSelector = true" :text="calEndTime"></x-button>
          </div>
        </div>
        <group v-show="false">
        ...
        <group v-show="false">
          <datetime v-model="calEndTime"
                    :show.sync="endSelector"
                    :format="'YYYY-MM-DD'"
                    :end-date="end_date"
                    @on-confirm="onConfirmEnd"></datetime>
        </group>
        <button class="fz-38 fc-ffffff bg-F7094D btn_radius"
                style="display: block;width: 100%;height: .9rem"
                @click="linkToDetail">确定</button>
      </div>
