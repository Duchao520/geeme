  ...
  <div class="mchpar">
    ...
    <div style="padding-bottom: 1.5rem">
      ...
      <group title="信息验证" label-width="2rem">
        <x-input title="手机号码" required v-model="formData.phone" placeholder="请输入手机号码" type="tel" keyboard="number" is-type="china-mobile"></x-input>
      ...
      <group title="商户信息" label-width="2rem">
        <x-input text-align="right" required title="商户名称" v-model="formData.shopShortName" placeholder="请输入商户名称" type="text"></x-input>
        ...
        <x-input title="门店名称" required v-model="formData.storeName" name="mobile" placeholder="请输入门店名称" type="text" text-align="right"></x-input>
        <x-input title="客服电话" required v-model="formData.tel" placeholder="请输入客服电话" keyboard="number" type="tel" text-align="right"></x-input>
        <x-input title="联系人电话" required v-model="formData.mobilePhone" name="mobile" placeholder="请输入联系人电话" keyboard="number" type="tel" is-type="china-mobile" text-align="right"></x-input>
        ...
        <popup-picker required placeholder="请选择所属行业" title="所属行业" :data="INDUSTRY" v-model="productDesc" ></popup-picker>
        ...
        <x-address title="所属城市" required id="storeAddress" :list="addressData"  placeholder="请选择所属城市" @on-shadow-change="getStoreAddress"></x-address>
        <x-input title="详细地址" required v-model="formData.detailAddress" placeholder="请输入详细地址" type="text" text-align="right"></x-input>
        <x-switch title="营业期限" inline-desc="若营业期限为永久请选中" @on-change="merTimeLong" v-model="merdateFlag"></x-switch>
        <div class="flex_box box_padding justify_space_between" style="padding: .25rem;border-top: .01rem solid #e0e0e0">
          ...
          <div class="flex_box">
            <div style="width: 1.7rem;border-bottom: .02rem solid #e0e0e0;">
              <datetime title="开始日期" :max-year=2050 clear-text="today" required style="padding: 0;width: 1.7rem;height: .5rem" v-model="merstartDate">
                <x-button class="fz-22" style="padding: 0">{{merstartDate}}</x-button>
              </datetime>
            </div>
            -
            <div style="width: 1.7rem;border-bottom: .02rem solid #e0e0e0;" v-if="!merdateFlag">
              <datetime title="结束日期" :max-year=2050 clear-text="today" style="padding: 0;width: 1.7rem;height: .5rem" v-model="merendDate">
                <x-button class="fz-22" style="padding: 0">{{merendDate}}</x-button>
              </datetime>
            </div>
      ...
      <group title="结算信息" label-width="2rem">
        ...
        <x-input type="text" required title="支付宝账户" v-model="formData.aliAccount" name="mobile" placeholder="请输入支付宝账户" text-align="right"></x-input>
        <x-switch title="身份证永久" inline-desc="若身份证为永久请选中" @on-change="idCardValidTimeLong" v-model="dateFlag"></x-switch>
      ...
      <group title="身份证照(法人)" label-width="2rem">
        <div class="box_padding flex_box justify_space_between" style="padding: .32rem">
          <div style="width: 3.26rem;height: 2.16rem;position: relative;border-radius: .1rem;overflow: hidden">
            <img :src="idCardCopy ? idCardCopy : require('@/assets/imgs/mchpar/idcard_just.jpg')" alt="" style="position: absolute;display: block;width: 3.26rem;height: 2.16rem;z-index: 1">
            <input type="file" required style="display: block;position: absolute;width: 3.26rem;height: 2.16rem;opacity: 0;z-index: 3" id="idCardCopy" @change="IMG_UPLOAD">
            <div class="box_padding" style="padding-top: .16rem;position: absolute;width: 3.26rem;height: 2.16rem;z-index: 2;background: rgba(12,12,12,.1);">
              ...
              <p class="text-center fc-ffffff fz-28">身份证(正面)</p>
            </div>
          </div>
          <div style="width: 3.26rem;height: 2.16rem;position: relative;border-radius: .1rem;overflow: hidden">
            <img :src="idCardNational ? idCardNational : require('@/assets/imgs/mchpar/idcard_against.jpg')" alt="" style="position: absolute;display: block;width: 3.26rem;height: 2.16rem;z-index: 1">
            <input type="file" required style="display: block;position: absolute;width: 3.26rem;height: 2.16rem;opacity: 0;z-index: 3" id="idCardNational" @change="IMG_UPLOAD">
            <div class="box_padding" style="padding-top: .16rem;position: absolute;width: 3.26rem;height: 2.16rem;z-index: 2;background: rgba(12,12,12,.1);">
              <p class="text-center" style="color: #dbdbdb;font-size: .64rem;margin-bottom: .24rem">camera</p>
              <p class="text-center fc-ffffff fz-28">身份证(反面)</p>
            </div>
          </div>
        </div>
      </group>
      <group title="门店校验照片" label-width="2rem">
        <div class="box_padding flex_box justify_space_between" style="padding: .32rem">
          <div style="width: 3.26rem;height: 2.16rem;position: relative;border-radius: .1rem;overflow: hidden">
            <img :src="storeEntrancePic ? storeEntrancePic : require('@/assets/imgs/mchpar/mentou.jpg')" alt="" style="position: absolute;display: block;width: 3.26rem;height: 2.16rem;z-index: 1">
            <input type="file" required style="display: block;position: absolute;width: 3.26rem;height: 2.16rem;opacity: 0;z-index: 3" id="storeEntrancePic" @change="IMG_UPLOAD">
            <div class="box_padding" style="padding-top: .16rem;position: absolute;width: 3.26rem;height: 2.16rem;z-index: 2;background: rgba(12,12,12,.1);">
              <p class="text-center" style="color: #dbdbdb;font-size: .64rem;margin-bottom: .24rem">camera</p>
              <p class="text-center fc-ffffff fz-28">门头照片</p>
            </div>
          </div>
          <div style="width: 3.26rem;height: 2.16rem;position: relative;border-radius: .1rem;overflow: hidden">
            <img :src="indoorPic ? indoorPic : require('@/assets/imgs/mchpar/neijing.jpg')" alt="" style="position: absolute;display: block;width: 3.26rem;height: 2.16rem;z-index: 1">
            <input type="file" required style="display: block;position: absolute;width: 3.26rem;height: 2.16rem;opacity: 0;z-index: 3" id="indoorPic" @change="IMG_UPLOAD">
            <div class="box_padding" style="padding-top: .16rem;position: absolute;width: 3.26rem;height: 2.16rem;z-index: 2;background: rgba(12,12,12,.1);">
              <p class="text-center" style="color: #dbdbdb;font-size: .64rem;margin-bottom: .24rem">camera</p>
              <p class="text-center fc-ffffff fz-28">门店内景</p>
            </div>
          </div>
        </div>
      </group>
      ...
      <group title="门店门面租赁合同扫描件或经营场地证明(非必传)" label-width="2rem">
        <div class="box_padding flex_box justify_space_between" style="padding: .32rem">
          <div style="width: 3.26rem;height: 2.16rem;position: relative;border-radius: .1rem;overflow: hidden">
            <img :src="addressCertification ? addressCertification : require('@/assets/imgs/mchpar/mentou.jpg')" alt="" style="position: absolute;display: block;width: 3.26rem;height: 2.16rem;z-index: 1">
            <input type="file" required style="display: block;position: absolute;width: 3.26rem;height: 2.16rem;opacity: 0;z-index: 3" id="addressCertification" @change="IMG_UPLOAD">
            <div class="box_padding" style="padding-top: .16rem;position: absolute;width: 3.26rem;height: 2.16rem;z-index: 2;background: rgba(12,12,12,.1);">
              <p class="text-center" style="color: #dbdbdb;font-size: .64rem;margin-bottom: .24rem">camera</p>
            </div>
          </div>
...
<script>
  import { XInput,Group,XButton , Selector ,PopupPicker,Datetime ,
    ...
    TransferDom, Popup} from 'vux';
  import BScroll from 'better-scroll';
  import {dataURLtoBlob} from "@/assets/js/deal";
  import axios from 'axios';
  import {getBankList , sendMsCode , mchparSave , getShopType, getAliType} from "@/assets/js/comm";
  export default {
    name: "mchpar",
    directives: {
      TransferDom
    ...
    data(){
      return{
        ...
        INDUSTRY: [['餐饮','线下零售','居民生活服务','休闲娱乐','交通出行','其他']],
    ...
    mounted() {
      var _self = this;
      window.document.title = '小微进件';
      // alert(this.$route.query.activeCode);
      this.getAliType();
      this.$nextTick(() => {
        this.initScroll();
      })
    ...
    watch: {
      startDate() {
        if(this.dateFlag){
          this.formData.idCardValidTime = this.startDate + ',' + '长期';
        }else{
          this.formData.idCardValidTime = this.startDate + ',' + this.endDate;
        }
      },
      endDate() {
        this.formData.idCardValidTime = this.startDate + ',' + this.endDate;
      ...
      productDesc(){
        this.formData.productDesc = this.productDesc[0];
      },
      accountBank(){
    ...
    methods: {
      ...
      getStoreAddress(areacodes,citys){
        areacodes.forEach(ele => {
          if(ele != '--'){
            this.formData.storeAddressCode = ele
          }
        });
        var address = citys.join('');
        this.formData.areaFullName = address + this.formData.detailAddress;
      },
      ...
      verifyFormData(){
        for(let i in this.formData){
          if(!this.formData[i] && i != 'shopId' && i != 'addressCertification' && i != 'addressCertificationMediaId' && i != 'businessAdditionPics' && i != 'businessAdditionPicsMediaId' && i != 'aliAccount' && i != 'activeCode'){
            switch (i) {
              case 'phone':
              ...
              case 'storeName':
                this.toast('请输入门店名称');break;
              case 'tel':
                this.toast('请输入客服电话');break;
              case 'mobilePhone':
                this.toast('请输入联系人电话');break;
              case 'productDesc':
                this.toast('请选择所属行业');break;
              case 'areaFullName':
                this.toast('请选择所属城市');break;
              case 'storeAddressCode':
                this.toast('请选择所属城市');break;
              case 'detailAddress':
                this.toast('请输入详细地址');break;
              case 'accountBank':
              ...
              case 'idCardNumber':
                this.toast('请输入身份证号');break;
              case 'idCardName':
                this.toast('请输入身份证姓名');break;
              case 'idCardValidTime':
                this.toast('请选择证件有效期');break;
              case 'bankName':
                this.toast('请输入开户支行');break;
              case 'bankAddressCode':
                this.toast('请选择开户地区');break;
              case 'idCardCopy':
                this.toast('请上传身份证正面照片');break;
              case 'idCardNational':
                this.toast('请上传身份证背面照片');break;
              case 'storeEntrancePic':
                this.toast('请上传门头照片');break;
              case 'indoorPic':
                this.toast('请上传门店内景照片');break;
              case 'mccCode':
                this.toast('请选择经营类目');break;
              case 'businessLicenseNumber':
                this.toast('请输入营业执照注册号');break;
              case 'businessTime':
            ...
            return false;
          }else if(i == 'phone' || i == 'mobilePhone'){
            if(!(/^(1)[0-9]{10}$/.test(this.formData[i]))){
              if(i == 'phone'){
                this.toast('请输入规范手机号')
              }else if(i == 'mobilePhone'){
                this.toast('请输入规范联系人电话')
              }
              return false;
            }
          }else if(i == 'idCardNumber'){
            var IDCARD_REG = /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/;
            if(!IDCARD_REG.test(this.formData[i])){
              this.toast('请输入规范身份证号');
              return false;
            }
          }else if(!this.startDate){
            this.toast('请选择身份证开始日期');
            return false;
          }else if(!this.merstartDate){
            this.toast('请选择营业期限开始日期');
            return false;
          }
        }
        return true;
      ...
      submit(){
        ...
        if(this.verifyFormData()){
          alert(this.formData.activeCode);
          for(let i in this.formData){
        ...
        if(_self.formData.phone){
          ...
          if(file.size > 1024 * 1024 * 2){
            ...
            reader.onload=function(e){
              ...
              formData.append('mobilePhone',_self.formData.phone);
