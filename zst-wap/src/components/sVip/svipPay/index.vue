<template>
  <div class="index" @click="isShowKeyboard(0); type = 0">
    ...
    <div class="pay">
      ...
      <div class="inputContainer" v-show="discountInput && priType == 1" :class="{active: type == 2}">
    ...
    <div class="pay card" @click="priType = 1" v-show="svipIsShow">
      <div class="justify_content dashed">
        <div class="left">
          ...
          <span>共享卡支付最低{{(minSvipDiscount*10).toFixed(1)}}折</span>
        </div>
    ...
    <div class="pay flex-box" @click="priType = 2" v-show="actIsShow">
    ...
    <div class="pay flex-box" style="justify-content: space-between" @click="priType = 3" v-show="couponIsShow">
      ...
      <span @click="showCouponList" v-if="coupon_list_length" :class="{'fc-F7094D': coupon}">{{coupon ? '-' + coupon.couponMoney + '元' : '共' + coupon_list_length + '张'}}<i
...
<script>
  ...
  import {getPayPage, getUserBalance, getsVipOrder, postActOrder, govippage} from "@/assets/js/comm";
  ...
  export default {
    ...
    data() {
      return {
        ...
        lngtitude: '',                        //精度
        latitude: '',                         //纬度
        storeId: this.$route.query.shopStoreId,                  //店铺storedId
        shareuserId: this.$route.query.shareuserId,
        shopId: '',
        scanSrc: this.$route.query.scanSrc,
    ...
    methods: {
      ...
      judgeAlert() {
        var _self = this;
        var discount = (Subtr(accMul(accDiv(6, _self.rate), 10), 6)) || 0;
        _self.def_status = 0;
        if (_self.rate && _self.totalMoney) {
          if (Number(_self.totalMoney) < 6 / _self.rate * 10) {
            _self.disMsg = '满' + Math.ceil(6 / _self.rate * 1000) / 100 + '元可抵扣' + Math.ceil(discount * 100) / 100 + '元,还差' + Math.ceil((6 / _self.rate * 10 - Number(_self.totalMoney)) * 100) / 100 + '元';
            _self.defActPay = Math.ceil(Number(_self.totalMoney) * 100);
            _self.defDiscount = 0;
          } else {
            if (_self.rate) {
              discount = Number(_self.totalMoney) - Number(_self.totalMoney) * _self.rate * 0.1;
              if (_self.user.totalmoney >= discount) {
                _self.defActPay = Math.ceil(Number(_self.totalMoney) * _self.rate * 0.1 * 100);
                _self.defDiscount = Subtr(accMul(_self.totalMoney, 100), _self.defActPay);
                _self.disMsg = '话呗抵扣 : ' + accDiv(_self.defDiscount, 100) + '元';
              ...
              _self.disMsg = '话呗抵扣 : ' + 0 + '元';
              _self.defActPay = Math.ceil(Number(_self.totalMoney) * 100);
              _self.defDiscount = 0;
              _self.defActiveNum = ''
            }
          }
        } else if (_self.reduAct.length != 0 && _self.totalMoney) {
          try {
            if (_self.user.totalmoney >= _self.reduAct[0].availableIntegral) {      //判断话呗余额是否达到最低满减
              if (_self.act) {
                if (_self.user.totalmoney >= _self.act.availableIntegral) {
                  _self.disMsg = "满" + _self.act.consumeMoney + "元减" + _self.act.availableIntegral * 10 / 10 + "元";
                  _self.defActPay = Math.ceil((Number(_self.totalMoney) - _self.act.availableIntegral) * 100);
                  _self.defDiscount = _self.act.availableIntegral * 100;
                ...
                _self.disMsg = "满" + obj.consumeMoney + "元减" + obj.availableIntegral * 10 / 10 + "元,当前还差" + (obj.consumeMoney - Number(_self.totalMoney)).toFixed(1) + '元';
                _self.defActPay = Math.ceil(accMul(_self.totalMoney, 100));
                _self.defDiscount = 0;
                _self.defActiveNum = '';
              }
            } else if (_self.totalMoney) {                                  //余额不足最低满减 提醒余额不足选择充值或原价付
              _self.disMsg = "话呗余额不足";
              _self.defActPay = Math.ceil(accMul(_self.totalMoney, 100));
              _self.defDiscount = 0;
              _self.defActiveNum = '';
          ...
          _self.disMsg = '';
          _self.defActPay = accMul(_self.totalMoney, 100);
          _self.defDiscount = 0;
          _self.defActiveNum = '';
        }
      },
      ...
      priJudge() {
        ...
        if (Number(_self.totalMoney)) {
          if (_self.priType == 1) {                    //卡主支付
            ...
            _self.pay();
          } else if (_self.priType == 3) {             //优惠券折扣满减
            _self.couponPay();
          } else {                                     //原价支付
      ...
      calMoney() {
        ...
        _self.disMoney = Subtr(Number(_self.totalMoney), Number(_self.notDisMoney));
        //卡主支付
        if(!CARDPAGE.shareCard){
          if(_self.mapCard()){
            ...
            _self.svipDiscount = Subtr(accMul(_self.totalMoney, 100), _self.svipActPay);
          }else{
            _self.svipActPay = accMul(_self.totalMoney,100);
            _self.svipDiscount = 0;
          }
        }else{
          _self.svipActPay = Math.ceil(accAdd(accMul(accMul(_self.disMoney, _self.cardDetail.shareCardDiscount ? _self.cardDetail.shareCardDiscount : 0), 100), accMul(_self.notDisMoney, 100)));
        ...
        if (_self.coupon) {                         //有优惠券时
          if (Number(_self.totalMoney) >= _self.coupon.consumeMoney * 100 / 100) {
            _self.couponActPay = Math.ceil(accMul(Subtr(Number(_self.totalMoney), _self.coupon.couponMoney), 100));
            _self.couponDiscount = Subtr(Number(_self.totalMoney) * 100, _self.couponActPay);
          } else {
      ...
      priTypeEvent() {
        var _self = this;
        if (_self.priType == 1) {
          _self.actPay = _self.svipActPay;
          _self.discount = _self.svipDiscount;
        } else if (_self.priType == 2) {
          _self.actPay = _self.defActPay;
          _self.discount = _self.defDiscount;
          _self.activeNum = _self.defActiveNum;
        } else if (_self.priType == 3) {
          _self.actPay = _self.couponActPay;
