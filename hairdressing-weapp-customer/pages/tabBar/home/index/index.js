...
Page({
  data: {
    ...
    // 组件数据(推荐美店)
    likelist: '',
    tabList: [{ showName: '全部', type: 0 }, { showName: '服务', type: 1 }, { showName: '产品', type: 2 }, { showName: '卡项', type: 3 }],
    // 作品动态
    dynamicList: [],
  ...
  onLoad(options) {
    ...
    if(options.storeId){
      wx.$setStorage('storeId',options.storeId)
    }
    this.init();
  },
  onShow() {
  ...
  getShopInfo(shopInfo) {
    ...
    util.PromiseAll([this.getAdBannerByStoreId(), this.getStoreWholeGoodsClass(), this.getGroupBuyProducts(), this.getShopServiceList(), this.weappPagedQueryStoreAllCardKindBasicInfo(), this.weGetExperiencePriceGoodsList()]).then(res => {
      if (shopInfo.status == 'ONLINE' || shopInfo.status == '营业中'){
        abnormal = true;
      }else{
        abnormal = false;
      }
      Object.assign(res, { BRAND_HOME_SWITCHER: false, shopInfo, browerStoreList, SHADE_SWITCHER: false, abnormal})
      this.setData(res);
    })
  },
  getShopServiceList() {
  ...
  weappPagedQueryStoreAllCardKindBasicInfo() {
    return apiAction.weappPagedQueryStoreAllCardKindBasicInfo().then(res => {
      if (res.success) {
        return {
          listCardKindBasicInfoSupperRecommend: res.listCardKindBasicInfoSupperRecommend
        }
      } else {
        return {}
      }
  ...
  pagedQueryStoreCardKindBasicInfo() {
    ...
    wx.showLoading()
    return apiAction.pagedQueryStoreCardKindBasicInfo(params).then(data => {
      wx.hideLoading()
      if (!data.listCardKindBasicInfo.length) {
        return {};
      }
      data.listCardKindBasicInfo.forEach(ele => {
        ele.type = 'card';
        ele.address = util.replaceCity(shopInfo.city) + ' ' + shopInfo.shopName
      })
      return {
        productList: data.listCardKindBasicInfo
      }
    })
  },
        ...
        res.columnSettingSelfApiList.forEach(ele => {
          switch(ele.name){
            ...
            case '新人专享':
              ...
              break;
            case '充值有礼':
              HOME_SECOND_FILTER.rechargeGift = ele;
              break;
          }
        })
  ...
  selectGoodsType(e) {
    ...
    switch (Number(type)) {
      ...
      case 2:
        ...
        break;
      case 3:
        promise = this.pagedQueryStoreCardKindBasicInfo();
        break;
    }
    promise.then(res => {
      this.$setData(res)
  ...
  onPullDownRefresh() {
    ...
    util.PromiseAll([this.getAdBannerByStoreId(), this.getStoreWholeGoodsClass(), this.getGroupBuyProducts(), promise, this.weappPagedQueryStoreAllCardKindBasicInfo(), this.weGetExperiencePriceGoodsList()]).then(res => {
      this.setData(res);
    })
  },
  //打开下拉最近浏览列表
  ...
  selectStore(e) {
    ...
    if (type == 'selectBrand') {
      this.setData({ BRAND_HOME_SWITCHER: true, SHADE_SWITCHER: false, IS_MATCH: false })
   ...
   * 品牌中心页面
   */
  brandJumpTabDefault(e) {
    let { type } = e.currentTarget.dataset, url
    switch (type) {
      case 'DISCOUNT':
        url = '/pages/IC/pages/newCustomer/newCustomer'; break;
      case 'SUGGEST_STORE':
        url = '/pages/SC/pages/recommended/recommended'; break;
      case 'ASSESSMENT':
        url = '/pages/IC/pages/customerAssess/customerAssess'; break;
      case 'BRAND_HOME':
        url = '/pages/tabBar/brandCenter/index/index'; break;
    }
    wx.$route(url);
  },
