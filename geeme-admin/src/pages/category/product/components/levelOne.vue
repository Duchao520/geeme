<template>
  <a-modal :visible="visible" :title="title" @cancel="onCancel" :destroyOnClose=true width="800px">
    ...
    <IconUpload :groupKey="2" ref="iconUpload" v-bind="form.icon" @success="onSuccess"/>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        if (this.operation === 'add') {
          res = await addProductClass(param)
        } else if (this.operation === 'edit') {
          res = await editProductClass(param)
        }
      } finally {
        this.loading = false
      }
    ...
    /* 获取分类详情 */
    async retrive() {
      const res = await retriveProductClass(this.id);
      Object.keys(this.form).forEach(i => {
        this.form[i].initValue = res.data[i]
        this.form[i].value = this.form[i].initValue
        // 行业列表
        this.professionList.initValue = (res.data.professionList || []).map(i => i = i.id)
        this.professionList.value = this.professionList.initValue
      });
