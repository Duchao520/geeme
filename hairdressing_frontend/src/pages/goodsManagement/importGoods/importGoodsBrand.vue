    ...
    <div class="header-btn-line">
      <div class="left-btn">
        <el-upload
          class="upload-demo"
          accept=".xls"
          :action="seriviceTemAction"
          :data="{brandId: currentInfo.brandId}"
          :headers="{Authorization: token}"
          :file-list="fileList"
          :on-success="(res) => {handleOnSuccess(res, '4')}"
          :limit="1"
        >
          <el-button class="resize-btn normal_red_btn" :loading="btnLoad">导入服务模板信息</el-button>
        </el-upload>
        <el-upload
          class="upload-demo"
          accept=".xls"
          :action="productTemAction"
          :data="{brandId: currentInfo.brandId}"
          :headers="{Authorization: token}"
          :file-list="fileList"
          :on-success="(res) => {handleOnSuccess(res, '3')}"
          :limit="1"
        >
          <el-button class="resize-btn normal_green_btn" :loading="btnLoad">导入产品模板信息</el-button>
        </el-upload>
      </div>
      <div class="right-btn">
...
export default {
  ...
  data() {
    return {
      ...
      pageNum: 1,
      pageSize: 10,
      seriviceTemAction: `${process.env.SERVICE_URL}/service-api/importServiceTemps`,
      productTemAction: `${process.env.SERVICE_URL}/service-api/importProductTemps`,
      token: "",
      fileList: [], // 需求清空文件
      loading: false,
      btnLoad: false,
  ...
  methods: {
    getList(type) {
      ...
      axios
        .post("/getGoodsImportRecordList", {
          ...
          type: Number(this.currentStatus)
        })
        .then(res => {
    ...
    downloadTemplate(type) {
      ...
      let api = type === 'service' ? '/downloadServiceTempModel' : '/downloadProductModel'
      axios.get(api, { responseType: "blob" }).then(res => {
        let blob = new Blob([res], { type: "application/vnd.ms-excel" });
        let a = document.createElement("a");
        let url = (a.href = window.URL.createObjectURL(blob));
        a.download = type === 'service' ? '服务模板信息模板.xls' : '产品模板信息模板.xls'
