<template>
  <div class="root_" v-loading="pageLoading">
    <div class="root-con" v-if="tableData&&tableData.length>0">
      <div class="top">
        <span class="message">
          <i class="icon iconfont icon-icon_note"></i>注：产品分类只能通过总部或门店新增产品同步，不可删减和修改。
        </span>
        <input-search
          width="280px"
          placeholder="请输入分类名称"
          v-model="sortName"
          @search="getProductSortListByNameAndBrandId()"
        ></input-search>
      </div>
              ...
              >
                ...
                <div class="left">
                  ...
                  <img
                    ...
                    :preview-text="''"
                    style="cursor: pointer;"
                    src="@/assets/img/default_cover_04.png"
                    :alt="sortTwoItem.sortName"
                  />
                </div>
    ...
    <goods-sort-default
      ...
      text="暂时没有分类，总部或门店新增产品之后，分类信息将同步至此"
    ></goods-sort-default>
    <!-- 编辑对话框 -->
    <el-dialog
      title="编辑产品分类"
...
export default {
  ...
  methods: {
    init() {
      this.getProductSortListByNameAndBrandId();
    },
    getProductSortListByNameAndBrandId() {
      this.pageLoading = true;
      axios
        .post(api.getProductSortListByNameAndBrandId.URL, {
    ...
    updateSort(data) {
      axios
        .post("/updateProductSort", {
          id: data.id,
          orderNo: data.orderNo,
          recommendation: null
        })
        .then(res => {
          this.updateProductSortRes(res);
        });
    },
    changeRecommend(data) {
      axios.post("/updateProductSort", {
        id: data.id,
        orderNo: null,
        recommendation: data.recommendation
      });
    },
    updateProductSortRes(res) {
      if (res.success) {
    ...
    sendData() {
      this.$refs.editForm.validate(valid => {
        ...
        axios
          .post("/updateProductSort", this.editForm)
          .then(res => {
            if (!res.success) {
