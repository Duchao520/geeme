  ...
  <div class="open-up-page text-for-copy">
    ...
    <div class="content-wrap">
      <div class="content">
        ...
        <div class="form-info">
          <el-form 
            ref="paymentAccountSubmissionInfo" 
            :rules="formRules" 
            :model="paymentAccountSubmissionInfo" 
            :disabled="wechatPay.operate === 'detail'"
            label-width="120px"
          >
            ...
            <div class="form-box">
              ...
              <el-form-item label="经营类目" prop="categoryId">
                <p class="tips">示例选择：个体工商户 > 休闲娱乐 > 美发/美容/美甲店 <a target="_blank"
              ...
              <el-form-item label="门店地址" prop="storeAddress"
                ...
                <el-input v-model="paymentAccountSubmissionInfo.storeAddress" placeholder="请输入门店地址"
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState({
        ...
        wechatPay: state => state.payTypesAuth.wechatPay
      }),
      uploadUrl() {
      ...
      isShop() {
        return this.$route.path.indexOf("shop") !== -1 || this.wechatPay.isShop
      }
    },
    watch:{
    ...
    methods: {
      getDetail() {
        if (this.wechatPay.operate !== 'create' && this.wechatPay.id) { // 编辑&详情都要请求
          axios.post("/getSubmitDetail", {
            id: this.wechatPay.id
          }).then(res => {
            let arr = res.submitDetailInfo.supplementaryMaterials.split(",").map(item => {
              return {
                  name: `${Math.random()}`,
            ...
            arr =  arr[0].url.substr(0,1) === 'h' ? arr : [],
            console.log(arr)
            this.paymentAccountSubmissionInfo = {
              ...res.submitDetailInfo,
              supplementaryMaterialImages: arr,
      ...
      confirmSubmit() {
        ...
        this.$refs.paymentAccountSubmissionInfo.validate(valid => {
          if (valid) {
            console.log(this.paymentAccountSubmissionInfo.supplementaryMaterialImages)
