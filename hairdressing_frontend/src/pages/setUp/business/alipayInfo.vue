<template>
  <full-single-page :title="'开通支付宝支付'">
    <div class="alipay-page-body" v-loading="loading">
      <div class="warning-line" v-if="ruleForm.failReason && aliPay.operate === 'edit'">
				<i class="icon iconfont icon-icon_ope_cancel"></i>
				驳回原因: {{ruleForm.failReason}}
			</div>
      <el-form 
        :model="ruleForm" 
        :rules="rules" 
        ref="ruleForm" 
        label-width="100px"
        :disabled="aliPay.operate === 'detail'"
      >
        <h3>结算账户</h3>
        <div class="info-box">
          <el-form-item
        ...
        <div class="info-box">
          ...
          >
            <el-row>
              <el-col :span="4">
                <el-input v-model="ruleForm.contactName" placeholder="请输入姓名" maxlength="64"></el-input>
              </el-col>
            </el-row>
          ...
          >
            <el-row>
              <el-col :span="4">
                <el-input v-model="ruleForm.telephone" placeholder="请输入手机号码" maxlength="36"></el-input>
              </el-col>
            </el-row>
        ...
        <div class="info-box">
          ...
          >
            ...
            <!-- </template> -->
            <!-- <img :src="utils.formatPicImg(ruleForm.shopPhoto)" v-else alt=""> -->
          </el-form-item>
        </div>
      </el-form>
      <div class="bottom-line" v-if="aliPay.operate !== 'detail'">
...
export default {
  ...
  methods: {
    ...
    // - 
    getDetail() {
      if (this.aliPay.operate !== 'create' && this.aliPay.id) { // 编辑&详情都要请求
        axios.post("/getSubmitDetail", {
          id: this.aliPay.id
        }).then(res => {
          this.ruleForm = res.submitDetailInfo
          this.fileList = res.submitDetailInfo.shopPhoto.split(',').map(i => {
            return {
              name: `${Math.random()}`,
  ...
  computed: {
      ...
      isShop() {
        return this.$route.path.indexOf("shop") !== -1 || this.aliPay.isShop
      }
    },
}
</script>
...
.alipay-page-body {
  ...
  .warning-line {
    ...
    margin-bottom: 12px;
    border-radius: 3px;
    background-color: rgba(254, 234, 238, 1);
    border: 1px solid rgba(245, 45, 86, 1);
    overflow: hidden;
