...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      this.tableData = []
      let res = await axios.post('/newGetCommissionServiceList', data)
      this.tableLoading = false
      if (res.commissionServiceInfoList.length) {
        this.tableData = res.commissionServiceInfoList || []
    ...
    handleMultipleSetCommission() {
      ...
      if (!this.selectedRows.length) { 
        ...
        let r = this.selectedRows.some(i => {return !!i.id})
        if (r) {
    ...
    // v2 改成新增和编辑都去以前的编辑页面
    toSetComssionPage() {  
      this.$router.push(`/${this.currentEntry}-inner/edit-commission-plan-service`)
    }
  },
  computed: {
    ...mapState({
