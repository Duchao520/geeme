    ...
    <div class="edit-commission-plans" v-loading="loading">
      <el-form  :model="formData" :rules="rules" ref="ruleForm" label-width="110px">
        <el-form-item label="适用职务" required="">
          {{cardScheme.postName || cardScheme.postInfoList.map(i => i.postName).join('; ')}}
        </el-form-item>
      ...
      <div class="commission-body-table">
        ...
        <template v-if="active === 'sub-fixed'">
          ...
          <el-table class="sub-table" :data="tableData" border useVirtual height="616" :row-height="48" :excess-rows="8">
            ...
            <el-table-column prop="cardName" label="卡项名称" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>
                  {{scope.row.cardName}}
                </span>
            ...
            <el-table-column prop="operateType" label="开卡/续卡" width="100">
              <template slot-scope="scope">
            ...
            <el-table-column prop="commission" label="提成" width="140">
              <template slot-scope="scope">
                <div class="cost-input-box">
                  <input class="cost-input-before" maxlength="12" v-model.number="scope.row.commission" placeholder="提成">
    ...
    >
      <div class="fixed-dialog-body">
        <el-table :data="templateTable" :header-cell-style="headerRowStyle" border :span-method="objectSpanMethod" v-loading="setTemplateLoading">
          <el-table-column prop="cardType" label="卡项分类">
            <template slot-scope="scope">
              <span>{{templateCommonInfo.cardType|cardTypeFilter}}</span><br>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="performanceType" label="业绩计算方式" width="280" >
            <template slot-scope="scope">
              <div v-if="templateCommonInfo.cardType === 1 && scope.row.payType !== 1">-</div>
              <div class="performance-type-box" v-else>
                <!-- 支付方式为赠送时: 后端给出了默认的计算方式为 固定 ; 模板中也要固定-->
                <el-select 
                  v-model="scope.row.performanceType" 
          ...
          <el-table-column prop="commissionType" label="提成方式" width="140">
            <template slot-scope="scope">
              <div v-if="templateCommonInfo.cardType === 1 && scope.row.payType !== 1">-</div>
              <el-select v-model="scope.row.commissionType" placeholder="提成分配" v-else>
                <el-option label='比例提成' :value="1"></el-option>
                <el-option label='固定提成' :value="2"></el-option>
              </el-select>
            </template>
          ...
          <el-table-column prop="commission" label="提成" width="140">
            <template slot-scope="scope">
              <div v-if="templateCommonInfo.cardType === 1 && scope.row.payType !== 1">-</div>
              <div class="cost-input-box" v-else>
                <input class="cost-input-before" maxlength="12" v-model.number="scope.row.commission" placeholder="提成">
                <span  class="cost-input-after">{{scope.row.commissionType === 1 ? '%' : '元'}}</span>
              </div>
            </template>
...
export default {
  ...
  created() {
    if (this.cardScheme.type === 'create') {
      this.initCreatedTableData()
    } else {
      this.initCreatedTableData(this.cardScheme.id)
      this.formData.name = this.cardScheme.name
      this.formData.computeType = this.cardScheme.computeType
    }
  },
  methods: {
    // 新增固定提成方案
    initCreatedTableData(id) {
      ...
      // this.secondColSpan = []
      // this.threeColSpan = []
      axios.post('/newGetCommissionCardDetail', {brandId: this.currentInfo.brandId , storeId: this.currentInfo.shopId, commissionCardId: id}).then(res => {
        this.tableData = res.listNewCommissionCardDetailInfo
        // this.setSpanData(this.tableData, this.firstColSpan, 'cardType')
        // this.setSpanData(this.tableData, this.secondColSpan, 'cardId')
        // this.setSpanData(this.tableData, this.threeColSpan, 'operateType')
        this.loading = false
      })
    },
    // - 
    save() {
      for(let i = 0, len = this.tableData.length; i < len; i++) {
        ...
        } else if (this.tableData[i].commissionType === 1 && (!reg.percent.test(this.tableData[i].commission) && !!this.tableData[i].commission)) {
          // 如果提成方式是比例，校验比例值
      ...
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          let data = JSON.parse(JSON.stringify(this.cardScheme))
          delete data.type
          delete data.name
          this.btnLoading = true
          this.loading = true
          let url = '/newSaveOrUpdateCommissionCard'
          if (this.cardScheme.postInfoList.length && this.cardScheme.type === 'create') {
            // 批量新增 - 这里接口名字和服务、产品不一样
            url = '/batchCreateCommissionCard'
          }
          axios.post(url, {
            operateType: this.cardScheme.type === 'create' ? 1 : 2,
            listNewCommissionCardDetailInfo: this.tableData, 
            newCommissionCardInfo: {
              ...data,
              ...this.formData, 
            ...
            this.loading = false
            this.$message({type: res.success ? 'success' : 'error', message: res.message || '操作成功'})
            this.close()
          }).catch(err => {
            this.btnLoading = false
            this.loading = false
    ...
    saveFixedModel() {
      ...
      for(let i = 0, len = this.templateTable.length; i < len; i++) {
        if (this.templateTable[i].performanceType === 3 && !reg.discount.test(this.templateTable[i].performance) && !!this.templateTable[i].performance ) {
          // 如果是折扣计算方式，校验折扣数值
          return this.$message({ type: 'error', message: '请输入0~10的折扣率, 最多2位小数, 10表示不打折' })
        } else if (this.templateTable[i].commissionType === 1 && (!reg.percent.test(this.templateTable[i].commission) && !!this.templateTable[i].commission)) {
          // 如果提成方式是比例，校验提成比例值
          return this.$message({ type: 'error', message: '请输入0~100的百分比, 最多2位小数' })
        } else {
          payTypeObj[`${this.templateTable[i].payType}-${this.templateTable[i].operateType}`] = this.templateTable[i]
        }
      }
      ...
      // 取出匹配的数据并覆盖
      let target = this.tableData.filter(i => { return i.cardType === this.templateCommonInfo.cardType })
      target.map(i => Object.assign(i, payTypeObj[`${i.payType}-${i.operateType}`]))
  ...
  computed: {
    ...mapState({
      ...
      cardScheme: state => state.commission.cardScheme, // 产品提成方案...
    }),
    title() {
      let type = this.cardScheme.type === 'create' ? '新增' : '编辑'
      return `${type}提成方案-卡项`
    },
    // 以后的组件激活
    active() {
