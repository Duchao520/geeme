    ...
    <div class="dialog-body-filter-line">
      <div class="level-filter-box">
        <el-button 
          ...
          v-if="checkHasBtnPermission('/brand-inner/add-new-commission-plan-card', 'path')" 
        ...
        <el-table-column prop="defaultScheme" label="设为默认" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-radio 
              ...
              :label="true"
              :disabled="!checkHasBtnPermission('/brand-inner/edit-commission-plan-card', 'path')"
            >设为默认</el-radio>
          </template>
        </el-table-column>
        <el-table-column label="操作" show-overflow-tooltip v-if="checkHasBtnPermission('/brand-inner/edit-commission-plan-card', 'path')" >
          <template slot-scope="scope">
           <g-button
              :name="'编辑'"
              :type="'detail'"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.loading = true
      this.firstColSpan = []
      axios.post('/commissionCardList', params).then(res => {
        this.loading = false
    ...
    setDefaultScheme(row) {
      ...
      axios.put('/setDefaultCommissionCard', params).then(res => {
    ...
    editCommission(row) {
      ...
      this.$router.push('/brand-inner/edit-commission-plan-card')
    },
    // 搜索
    ...
    toAddProductCommission() {
      this.$router.push('/brand-inner/add-new-commission-plan-card')
    }
  },
  computed: {
    ...mapState({
