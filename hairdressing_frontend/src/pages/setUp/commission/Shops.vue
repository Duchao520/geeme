    ...
    <div class="dialog-body-filter-line">
      <div class="level-filter-box">
        <el-button 
          ...
          v-if="checkHasBtnPermission('/shop-inner/add-new-commission-plan-store', 'path')" 
          @click="toAddStoreCommission"
        >新增提成方案</el-button>
        <i v-else></i>
      </div>
        ...
        <el-table-column prop="defaultScheme" label="设为默认" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-radio 
              ...
              :label="true"
              :disabled="!checkHasBtnPermission('/shop-inner/edit-commission-plan-store', 'path')"
            >设为默认</el-radio>
          </template>
        </el-table-column>
        <el-table-column label="操作" show-overflow-tooltip v-if="checkHasBtnPermission('/shop-inner/edit-commission-plan-store', 'path')" >
          <template slot-scope="scope">
           <g-button
              :name="'编辑'"
              :type="'detail'"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.loading = true
      this.firstColSpan = []
      axios.post('/commissionStoreList', params).then(res => {
        this.loading = false
        this.tableData = res.commissionStoreListInfoList
    ...
    setDefaultScheme(row) {
      ...
      let params = {
        commissionStoreId: row.id,
        positionId: row.positionId,
        storeId: this.currentInfo.shopId
      }
      axios.put('/setDefaultCommissionStore', params).then(res => {
    ...
    // 编辑方案
    editCommission(row) {
      this.setShopScheme({id: row.id, 'id_desc': 'id是编辑门店提成方案的查询id'})
      this.$router.push('/shop-inner/edit-commission-plan-store')
    },
    // 搜索
    ...
    toAddStoreCommission() {
      this.$router.push('/shop-inner/add-new-commission-plan-store')
    }
  },
  computed: {
    ...mapState({
