<template>
  <div class="commission-edit-box">
    <seed-header :title="pageStatus === 'edit' ? '编辑提成方案-服务' : '新增提成方案-服务'" :hasTabs="false"></seed-header>
      ...
      <div class="commission-edit-container">
        <div class="commission-edit-body" v-loading="loading">
          <el-form  :model="formData" :rules="rules" ref="ruleForm" >
            ...
            <!-- 这个是提成类别: 劳动提成|销售提成 ; 表格里是提成方式: 比例提成|固定提成 -->
            <el-form-item :label-width="labelWidth" label="提成类别" prop="commissionType">
              <el-radio v-model="formData.commissionType" :label="1">劳动提成</el-radio>
              <el-radio v-model="formData.commissionType" :label="2" disabled>销售提成</el-radio>
            </el-form-item>
          ...
          <div class="commission-body-table">
            ...
            <template v-if="active === 'sub-fixed'">
              <el-table class="sub-table" :data="tableData" border :span-method="arraySpanMethod">
                <el-table-column prop="serviceLevelOne" label="一级分类">
                  <template slot-scope="scope">
                    <span>{{scope.row.serviceLevelOneName}}</span><br>
                    <span class="volume-btn" @click="volumeSet_fixed(scope.row, 'One')">批量设置</span>
                  </template>
                </el-table-column>
                <el-table-column prop="serviceLevelTwo" label="二级分类">
                  <template slot-scope="scope">
                    <span>{{scope.row.serviceLevelTwoName}}</span><br>
                    <span class="volume-btn" @click="volumeSet_fixed(scope.row, 'Two')">批量设置</span>
                  </template>
                </el-table-column>
                <el-table-column prop="simpleTitle" label="服务名称" show-overflow-tooltip></el-table-column>
                <el-table-column prop="minStorePrice" label="门店价（元）" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column prop="memberSpecified" label="普通会员(指定)">
                  <template slot-scope="scope">
                    <div class="cost-input-box">
                      <input class="cost-input-before" maxlength="12" v-model.number="scope.row.memberSpecified">
                      <span  class="cost-input-after">{{scope.row.commissionType === 1 ? '%' : '元'}}</span>
                    </div>
                  </template>
                </el-table-column>
                <el-table-column prop="memberNoneSpecified" label="普通会员(非指定)">
                  <template slot-scope="scope">
                    <div class="cost-input-box">
                      <input class="cost-input-before" maxlength="12" v-model.number="scope.row.memberNoneSpecified">
                      <span  class="cost-input-after">{{scope.row.commissionType === 1 ? '%' : '元'}}</span>
                    </div>
                  </template>
                </el-table-column>
                <!-- <el-table-column prop="memberIdentity" label="身份会员">
                  <template slot-scope="scope">
                    <div class="cost-input-box">
                      <input class="cost-input-before" maxlength="12" v-model.number="scope.row.memberIdentity">
                      <span  class="cost-input-after">{{scope.row.commissionType === 1 ? '%' : '元'}}</span>
                    </div>
                  </template>
                </el-table-column> -->
    ...
    <el-dialog class="fixed-dialog-box" title='批量设置提成' :visible.sync="fixedDialogVisible" width="720px">
      <div class="fixed-dialog-body">
        <div style="margin-bottom:10px;">分类: {{volumeTitle}}</div>
        <el-table :data="currentVolume_fixed" :header-cell-style="headerRowStyle">
          ...
          <!-- 通用模板接口用第几个输入框标识 -->
          <el-table-column prop="firstValue" label="普通会员(指定)">
            <template slot-scope="scope">
              <div class="cost-input-box">
          ...
          <el-table-column prop="secondValue" label="普通会员(非指定)">
            <template slot-scope="scope">
              <div class="cost-input-box">
          ...
          <!-- <el-table-column prop="memberIdentity" label="身份会员"> -->
          <!-- 通用模板接口用第几个输入框标识 -->
          <!-- <el-table-column prop="secondValue" label="身份会员">
            <template slot-scope="scope">
              <div class="cost-input-box">
                <input class="cost-input-before" maxlength="12" v-model.number="scope.row.secondValue">
                <span  class="cost-input-after">{{scope.row.commissionType === 1 ? '%' : '元'}}</span>
        ...
        <div class="dialog-inner-info">批量设置后，该分类下未来新增的服务项目将默认采用该提成比例。</div>
        <div class="dialog-inner-info" style="color:#333;" v-if="currentColIndex==='One'">
          <el-checkbox v-model="currentVolume_fixed[0].coveringSubClass"/>
          &nbsp;&nbsp;将本次修改同步到该分类下现有的所有二级分类。
        </div>
...
export default {
  data() {
    return {
      ...
      pageStatus: '', // 新增 编辑
      staffList: [],
      formData: { name: '', commissionType: 1, computeType: 1, positionId: null },
      // 以下[固定式]子组件使用
      ...
      rules: {
        ...
        commissionType: [{required: true, message: '请选择提成类别', trigger: 'change'}],
  ...
  methods: {
    ...
    initFixedTableInAdd() {
      ...
      this.firstColSpan = []
      this.secondColSpan = []
      axios.get('/getStoreServiceListForCommission/' + this.currentInfo.shopId).then(res => {
        this.tableData = res.serviceInfoForCommissions
        this.setSpanData(this.tableData, this.firstColSpan, 'serviceLevelOneId')
        this.setSpanData(this.tableData, this.secondColSpan, 'serviceLevelTwoId')
        this.loading = false
      })
    },
    // 编辑时
    getCommissionData() {
      ...
      this.firstColSpan = []
      this.secondColSpan = []
      axios.post('/getCommissionServiceById',{storeId: this.currentInfo.shopId, commissionServiceId: this.serviceScheme.id}).then(res => {
       
        this.formData = {
          name: res.name,
          computeType: res.computeType, 
          commissionType: res.commissionType, // 这个是提成类别: 劳动提成|销售提成 ; 表格里是提成方式: 比例提成|固定提成
    ...
    volumeSet_fixed(row, col) {
      ...
      this.volumeTitle = col === 'One' ? row.serviceLevelOneName : row.serviceLevelOneName + ' / ' + row.serviceLevelTwoName
      // 部分数据从模板取: 1、提成方式 2、普通会员 3、身份会员
      let params = {
        classId: row['serviceLevel' + col + 'Id'],
        storeId: this.currentInfo.shopId,
        parentId: col === 'One' ? null : row.serviceLevelOneId,
        type: "service"
      }
      this.loading = true
      axios.post('/getCommissionTemplate', params).then(res => {
        ...
        this.currentVolume_fixed = [{
          ...
          serviceLevelOneId: row.serviceLevelOneId,
          serviceLevelOneName: row.serviceLevelOneName,
          serviceLevelTwoId: row.serviceLevelTwoId,
          serviceLevelTwoName: row.serviceLevelTwoName,
          minMemberPrice: row.minMemberPrice,
          minStorePrice: row.minStorePrice,
        }]
    ...
    saveFixedModel() {
      ...
      let params = {
        ...
        coveringSubClass: templateData.coveringSubClass,
        parentClassId: this.currentColIndex === 'One' ? 0 : templateData.serviceLevelOneId,
        classId: this.currentColIndex === 'One' ? templateData.serviceLevelOneId : templateData.serviceLevelTwoId,
        type: 'service'
      }
      // 如果二级分类没有设置过批量模板，需要前端将二级分类的id传给后端, 后端在去处理
      if (this.currentColIndex === 'One' && templateData.coveringSubClass) {
        let levelOneId = templateData['serviceLevel' + this.currentColIndex + 'Id']
        let children = new Set()
        let arr = []
        this.tableData.map(row => {
          if (row['serviceLevel' + this.currentColIndex + 'Id'] === levelOneId) {
            children.add(row['serviceLevelTwoId'])
          }
        })
        arr = [...children].join(',')
        Object.assign(params, {children: arr})
      }
      axios.post('/saveOrUpdateTemplate', params).then(res => {
        // this.tableData : 目标对象 // templateData : 模板对象 // this.currentColIndex : 第几列
        if (templateData.coveringSubClass || this.currentColIndex === 'Two') {
          let id = templateData['serviceLevel' + this.currentColIndex + 'Id']
          this.tableData.map(row => {
            if (row['serviceLevel' + this.currentColIndex + 'Id'] === id) {
              row.commissionType = templateData.commissionType
              row.memberSpecified = templateData.firstValue
              row.memberNoneSpecified = templateData.secondValue
            }
          })
        }
    ...
    **/
    ...
    save() {
      ...
      if (this.formData.computeType === 1) {
        let status = this.checkIfInputIsLegal(this.tableData, 'memberSpecified', 'memberNoneSpecified')
        if (status === 2) {
          utils.showTip(this, {msg: '提成比例需要在 0~100 之间的阿拉伯数字', type: 'error'})
          return
        } else if (status === 3) {
          ...
          if (valid) {
            ...
            this.tableData.map(i => {
             tableData.push({
               ...
               memberSpecified: i.memberSpecified,
               memberNoneSpecified: i.memberNoneSpecified,
            ...
            axios.post('/saveOrUpdateCommissionService', params).then(res => {
  ...
  computed: {
    ...mapState({
      ...
      serviceScheme: state => state.commission.serviceScheme, // 服务提成方案...
    }),
    // 以后的组件激活
    active() {
      let c = ''
