...
export default {
  ...
  methods: {
    ...
    initFixedTableInAdd() {
      this.loading = true
      this.firstColSpan = []
      axios.post('/getCardCommissionInfo', {brandId: this.currentInfo.brandId}).then(res => {
    ...
    getCommissionData() {
      ...
      axios.post('/getCommissionCardById/', data).then(res => {
        this.formData = {
          name: res.name,
      ...
      if (this.formData.computeType === 1) {
        ...
        this.$refs['ruleForm'].validate(valid => {
          if (valid) {
            ...
            axios.post('/saveOrUpdateCommissionCard', params).then(res => {
