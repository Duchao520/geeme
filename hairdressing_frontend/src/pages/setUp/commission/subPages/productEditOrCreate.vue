<template>
  <div class="commission-edit-box">
    <seed-header :title="pageStatus === 'edit' ? '编辑提成方案-产品' : '新增提成方案-产品'" :hasTabs="false"></seed-header>
          ...
          <div class="commission-body-table">
            ...
            <template v-if="active === 'sub-fixed'">
              <el-table class="sub-table" :data="tableData" border :span-method="arraySpanMethod">
                <el-table-column prop="productLevelOne" label="一级分类">
                  <template slot-scope="scope">
                    <span>{{scope.row.productLevelOneName}}</span><br>
                    <span class="volume-btn" @click="volumeSet_fixed(scope.row, 'One')">批量设置</span>
                  </template>
                </el-table-column>
                <el-table-column prop="productLevelTwo" label="二级分类">
                  <template slot-scope="scope">
                    <span>{{scope.row.productLevelTwoName}}</span><br>
                    <span class="volume-btn" @click="volumeSet_fixed(scope.row, 'Two')">批量设置</span>
                  </template>
                </el-table-column>
                <el-table-column prop="simpleTitle" label="产品名称" show-overflow-tooltip></el-table-column>
                <el-table-column prop="minMemberPrice" label="会员价（元）" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column prop="memberCommon" label="普通会员">
                  <template slot-scope="scope">
                    <div class="cost-input-box">
                      <input class="cost-input-before" maxlength="12" v-model.number="scope.row.memberCommon">
    ...
    <el-dialog class="fixed-dialog-box" title='批量设置提成' :visible.sync="fixedDialogVisible" width="720px">
      <div class="fixed-dialog-body">
        ...
        <el-table :data="currentVolume_fixed" :header-cell-style="headerRowStyle">
          <el-table-column prop="minMemberPrice" label="会员价（元）"></el-table-column>
          <el-table-column prop="commissionType" label="提成方式">
            <template slot-scope="scope">
          ...
          <!-- <el-table-column prop="memberCommon" label="普通会员"> -->
          <!-- 通用模板接口用第几个输入框标识 -->
          <el-table-column prop="firstValue" label="普通会员">
            <template slot-scope="scope">
              <div class="cost-input-box">
...
export default {
  ...
  methods: {
    ...
    initFixedTableInAdd() {
      ...
      this.firstColSpan = []
      this.secondColSpan = []
      axios.post('/esGetStoreProductList', {storeId: this.currentInfo.shopId}).then(res => {
        this.tableData = res.productInfoList
        this.setSpanData(this.tableData, this.firstColSpan, 'productLevelOne')
        this.setSpanData(this.tableData, this.secondColSpan, 'productLevelTwo')
        this.loading = false
      })
    },
    // 编辑时
    getCommissionData() {
      ...
      this.firstColSpan = []
      this.secondColSpan = []
      axios.get('/getCommissionProductById/' + this.productScheme.id).then(res => {
        this.formData = {
          name: res.name,
    ...
    volumeSet_fixed(row, col) {
      ...
      this.volumeTitle = col === 'One' ? row.productLevelOneName : row.productLevelOneName + ' / ' + row.productLevelTwoName
      // 部分数据从模板取: 1、提成方式 2、普通会员 3、身份会员
      let params = {
        classId: row['productLevel' + col],
        storeId: this.currentInfo.shopId,
        parentId: col === 'One' ? null : row.productLevelOne,
        type: "product"
      }
      this.loading = true
      axios.post('/getCommissionTemplate', params).then(res => {
        ...
        this.currentVolume_fixed = [{
          ...
          productLevelOne: row.productLevelOne,
          productLevelOneName: row.productLevelOneName,
          productLevelTwo: row.productLevelTwo,
          productLevelTwoName: row.productLevelTwoName,
          minMemberPrice: row.minMemberPrice,
          minShopPrice: row.minShopPrice,
        }]
    ...
    saveFixedModel() {
      ...
      let params = {
        ...
        parentClassId: this.currentColIndex === 'One' ? 0 : templateData.productLevelOne,
        classId: this.currentColIndex === 'One' ? templateData.productLevelOne : templateData.productLevelTwo,
        type: 'product'
      }
      // 如果二级分类没有设置过批量模板，需要前端将二级分类的id传给后端, 后端在去处理
      if (this.currentColIndex === 'One' && templateData.coveringSubClass) {
        let levelOneId = templateData['productLevel' + this.currentColIndex]
        let children = new Set()
        let arr = []
        this.tableData.map(row => {
          if (row['productLevel' + this.currentColIndex] === levelOneId) {
            children.add(row['productLevelTwo'])
          }
        })
        arr = [...children].join(',')
      ...
      axios.post('/saveOrUpdateTemplate', params).then(res => {
        ...
        if (templateData.coveringSubClass || this.currentColIndex === 'Two') {
          let id = templateData['productLevel' + this.currentColIndex]
          this.tableData.map(row => {
            if (row['productLevel' + this.currentColIndex] === id) {
              row.commissionType = templateData.commissionType
              row.memberCommon = templateData.firstValue
              row.memberIdentity = templateData.secondValue
            }
          })
        }
    ...
    **/
    ...
    save() {
      ...
      if (this.formData.computeType === 1) {
        let status = this.checkIfInputIsLegal(this.tableData, 'memberCommon', 'memberIdentity')
        if (status === 2) {
          utils.showTip(this, {msg: '提成比例需要在 0~100 之间的阿拉伯数字', type: 'error'})
          return
        } else if (status === 3) {
        ...
        this.$refs['ruleForm'].validate(valid => {
          if (valid) {
            params = {
              ...this.formData,
              productDetailInfos: this.tableData,
              brandId: this.currentInfo.brandId,
              storeId: this.currentInfo.shopId,
            }
            this.btnLoading = true
            axios.post('/saveOrUpdateCommissionProduct', params).then(res => {
