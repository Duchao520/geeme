  ...
  <div>
    <div class="dialog-body-filter-line">
      <div class="level-filter-box">
        <el-button 
          ...
          v-if="checkHasBtnPermission('/shop-inner/add-new-commission-plan-service', 'path')" 
          @click="toAddServiceCommission"
        >新增提成方案</el-button>
        <i v-else></i>
      </div>
    ...
    <div class="main-con">
      <el-table v-loading="loading" :data="tableData" border :span-method="arraySpanMethod" height="calc(100vh - 225px)">
        ...
        <el-table-column prop="commissionType" label="提成类别" show-overflow-tooltip>
          <template slot-scope="scope">{{scope.row.commissionType|commissionTypeFilter}}</template>
        </el-table-column>
        ...
        <el-table-column prop="defaultScheme" label="设为默认" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-radio 
              ...
              :label="true"
              :disabled="!checkHasBtnPermission('/shop-inner/edit-commission-plan-service', 'path')"
            >设为默认</el-radio>
          </template>
        </el-table-column>
        <el-table-column label="操作" show-overflow-tooltip v-if="checkHasBtnPermission('/shop-inner/edit-commission-plan-service', 'path')" >
          <template slot-scope="scope">
            <g-button
              :name="'编辑'"
              :type="'detail'"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.loading = true
      this.firstColSpan = []
      axios.post('/commissionServiceList', params).then(res => {
        this.loading = false
        this.tableData = res.commissionServiceListInfos
    ...
    setDefaultScheme(row) {
      ...
      let params = {
        commissionServiceId: row.id,
        positionId: row.positionId,
        storeId: this.currentInfo.shopId
      }
      axios.put('/setDefaultCommissionService', params).then(res => {
    ...
    // 编辑方案
    editCommission(row) {
      this.setServiceScheme({id: row.id, 'id_desc': 'id是编辑产品提成方案的查询id'})
      this.$router.push('/shop-inner/edit-commission-plan-service')
    },
    // 搜索
    ...
    toAddServiceCommission() {
      this.$router.push('/shop-inner/add-new-commission-plan-service')
    }
  },
  computed: {
    ...mapState({
