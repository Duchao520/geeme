    ...
    <div class="dialog-body-filter-line">
      <div class="level-filter-box">
        <el-button 
          ...
          v-if="checkHasBtnPermission('/shop-inner/add-new-commission-plan-product', 'path')" 
        ...
        <el-table-column prop="defaultScheme" label="设为默认" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-radio 
              ...
              :label="true"
              :disabled="!checkHasBtnPermission('/shop-inner/edit-commission-plan-product', 'path')"
            >设为默认</el-radio>
          </template>
        </el-table-column>
        <el-table-column label="操作" show-overflow-tooltip v-if="checkHasBtnPermission('/shop-inner/edit-commission-plan-product', 'path')" >
          <template slot-scope="scope">
           <g-button
              :name="'编辑'"
              :type="'detail'"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.loading = true
      this.firstColSpan = []
      axios.post('/commissionProductList', params).then(res => {
        this.loading = false
        this.tableData = res.commissionProductListInfos
    ...
    setDefaultScheme(row) {
      ...
      let params = {
        commissionProductId: row.id,
        positionId: row.positionId,
        storeId: this.currentInfo.shopId
      }
      axios.put('/setDefaultCommissionProduct', params).then(res => {
    ...
    // 编辑方案
    editCommission(row) {
      this.setProductScheme({id: row.id, 'id_desc': 'id是编辑产品提成方案的查询id'})
      this.$router.push('/shop-inner/edit-commission-plan-product')
    },
    // 搜索
    ...
    toAddProductCommission() {
      this.$router.push('/shop-inner/add-new-commission-plan-product')
    }
  },
  computed: {
    ...mapState({
