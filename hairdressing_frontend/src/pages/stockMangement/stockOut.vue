<template>
  <div class="stock-out" v-loading="pageLoading">
    <div class="header-line">
      <div class="left-btns">
        <el-button 
          v-if="checkRolesAuthorityInPage('brand_stock_out_create')"
    ...
    <div class="filter-line">
      ...
      <div class="select-item">
        <span class="lable-title">出库类型: </span>
        <el-select
          clearable
          size="medium"
          placeholder="选择出库类型" 
    ...
    <div class="table-main">
      <div class="user-defined-tab">
        ...
        <div class="table-box">
          ...
          >
            <el-table-column label="出库单号" prop="stockOrderNo"></el-table-column>
            <el-table-column label="出库时间" prop="storageTime"></el-table-column>
            <el-table-column label="出库类型" prop="storageType">
              <template slot-scope="{row}">{{row.storageType | storageTypeFilter}}</template>
            </el-table-column>
            <!-- <el-table-column label="出库单金额" prop="listStockSubOrder"></el-table-column> -->
            <el-table-column label="出库摘要">
            ...
            <el-table-column label="操作">
              <template slot-scope="{row}">
                <!-- 改版后的订货出库 不能在此有操作按钮, 除非后端再给出字段 -->
                <div v-if="row.storageType !== 12">
                  <!-- 详情按钮或者审核  -->
                  <g-button
                    :name="`${row.deleted ? '明细' : row.orderStatus ? '明细' : '审核'}`"
                    type="detail"
                    v-if="row.deleted || row.orderStatus ? true : checkRolesAuthorityInPage('brand_stock_out_audit')"
                    :icon="`${row.deleted ? 'icon_operate_eyes' : row.orderStatus ? 'icon_operate_eyes' : 'icon_operate_edit'}`"
                    @click="todetailOrder(row)"
                  ></g-button>
                  <!-- 仅待审核可以删除 -->
                  <g-button
                    v-if="!row.deleted && !row.orderStatus && checkRolesAuthorityInPage('brand_stock_out_delete')"
                    :name="'删除'"
                    type="orange"
    ...
    >
      <div class="warning-color ">删除后，该出库单无法恢复!</div>
    </g-dialog-with-slot>
    <!-- 创建出库单 -->
    <enterGoods
      ref="enterRef" 
...
export default {
  data() {
    return {
      ...
      goodsDialogVisible: false, //  出库对话框
      timeValue: [], 
      ...
      tableData: [], // 
      storageType: null, // 出出库类型类型 选择器的值
      pageNum: 1,
      pageSize: 10,
      total: 0,

      detailInfo: {}, // 出出库单明细。 自己从表格行内取, 无接口
      detailDialog: false, // 出出库单明细弹窗
  ...
  methods: {
    ...
    // 出库单明细 - 根据 currentType 自己取当前的标签状态、再对弹窗作相应的功能限制
    todetailOrder(row) {
    ...
    // 删除出库单 - 确认
    deleteEnterOrder() {
      axios.post(`/deleteProductStockOutOrder`, {stockOrderId: this.currentOperateId}).then(res => {
        if (res.success) {
    ...
    async handleCreateEnterOrder(data) {
      let res = await axios.post('/createProductStockOutOrder', data)
      if (res.success) {
