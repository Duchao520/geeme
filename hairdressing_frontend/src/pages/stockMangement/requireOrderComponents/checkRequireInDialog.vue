<template>
  <div v-loading.fullscreen.lock="submitLoading">
      ...
      >
        ...
        <el-form-item label-width="40px">
          <el-table :data="form.listStockSubOrder" :height="form.listStockSubOrder.length > 10 ? '650px' : ''">
            ...
            <!-- 审核改名叫清点了 -->
            <el-table-column label="清点数量" prop="signNum">
    ...
    >
      <div class="warning-color">通过后，该入库单无法删除,且不可逆!</div>
    </g-dialog-with-slot>
...
export default {
  ...
  methods: {
    async init () {
      ...
      await axios.post('/getProductStockOrderDetailInfo', {stockOrderId: this.detail.inStockOrderId}).then(res => {
        if (res.success) {
          this.submitLoading = false
          res.stockOrderInfo.listStockSubOrder.map(i => {
            i.signNum = i.num // 签名数量默认设置成发货数量
          })
    ...
    async handleSignOrder() {
      let url = '/signProductStockOrderIn'
      this.submitLoading = true
