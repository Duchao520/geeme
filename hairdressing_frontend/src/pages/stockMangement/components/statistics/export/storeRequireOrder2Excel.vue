...
export default {
  ...
  methods: {
    async getAllData() {
      let url = '/getStoreInvoiceStatistics'
      let params = {
        ...this.params,
      ...
      if (res.success) {
        this.tableData = this.tableData.concat(res.listInvoiceStatisticsStore || [])
    ...
    templateForExcel() {
      let time = this.params.startDay ? `
        <tr>
          <th colspan="3" style="text-align: left;">统计时段: ${this.params.startDay.slice(0, 10)} 至 ${this.params.endDay.slice(0, 10)}</th>
        </tr>
      ` : ''
      let tableStr = `
        <tr style="text-align: center;">
          <th colspan="3">${this.title}</th>
        </tr>
        ${time}
        <tr style="text-align: center;">
          <th>门店名称</th>
