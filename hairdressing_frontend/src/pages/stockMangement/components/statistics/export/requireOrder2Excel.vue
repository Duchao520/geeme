...
export default {
  ...
  methods: {
    async getAllData() {
      let url = '/getPagedAllInvoiceStatistics'
      let params = {
        ...this.params,
      ...
      if (res.success) {
        this.tableData = this.tableData.concat(res.listInvoiceStatisticsDay || [])
    ...
    templateForExcel() {
      let time = this.params.startDay ? `
        <tr>
          <th colspan="4" style="text-align: left;">统计时段: ${this.params.startDay.slice(0, 10)} 至 ${this.params.endDay.slice(0, 10)}</th>
        </tr>
      ` : ''
      let tableStr = `
