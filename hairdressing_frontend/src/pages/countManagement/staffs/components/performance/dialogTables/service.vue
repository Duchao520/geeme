<template>
  ...
  >
    ...
    <el-table-column prop="isChosen" label="指定">
      <template slot-scope="scope">{{scope.row.isChosen ? '指定' : '非指定'}}</template>
    </el-table-column>
    <el-table-column prop="levelTwoName" label="消费分类"></el-table-column>
    <el-table-column label="消费项目">
      <template
        slot-scope="scope"
      >{{scope.row.serviceName}}{{scope.row.specificSnapName === '默认' ? '' : `-${scope.row.specificSnapName}` }}</template>
    </el-table-column>
