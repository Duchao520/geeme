<template>
  ...
  >
    <img
      ...
      :src="utils.formatPicImage(product.image, 'style', 146, 'someone', product.sex)"
    />
    <div class="text-info">
      ...
      <div class="bottom-text">
        <p class="item-price">
          ...
          <span class="shop-price">¥{{ product.shopPrice}}</span>
        </p>
        <p class="item-stock">
          <span>库存 {{SumStock}}</span>
        </p>
      </div>
    </div>
    <!-- 2个定位的标记(可能存在) -->
    <!-- 1: keyword 员工店内编号 -->
    <div class="keyword-info" v-if="product.keyword">{{product.keyword}}</div>
    <!-- 2: 开单人可提供的服务 -->
    <img
      class="icon-image-card"
      title="开单人可提供的服务"
      src="@/assets/img/icon_tag_star.png"
      v-if="product.operate && billInfo.billingStaff.staffId && billInfo.goodsViewWithImg === 'card'"
    />
    ...
    <img
      ...
      title="开单人可提供的服务"
      src="@/assets/img/icon_tag_star.png"
      v-if="product.operate && billInfo.billingStaff.staffId && billInfo.goodsViewWithImg === 'list'"
    />
    ...
    <multipleSpecification
      ...
      :type="product.goodsType"
      :specificationInfoList="product.specificationInfoList"
...
export default {
  ...
  methods: {
    ...
    chooseGoodsOrSpecification(event) {
      // let sum = this.SumStock(this.product.specificationInfoList)
      this.animation = () =>
        animation.animationEvent(event, 100, 0.4, this.billInfo.carPosition);
      if (this.product.specificationInfoList.length === 1) {
        // 产品库存数不足处理
        let flag = false;
        if (this.product.specificationInfoList[0].stock <= 0) {
          flag = true;
        }
        this.billInfo.selectedGoods.map(i => {
          if (this.product.specificationInfoList[0].id == i.specificationId) {
            if (this.product.specificationInfoList[0].stock <= i.count) {
              flag = true;
            }
          }
        });
        if (flag) return this.$message.error("库存不足");
        // this.product.specificationInfoList[0].stock =
    ...
    saveSpecificationToOrder(idx, num) {
      // this.product.specificationInfoList[idx].stock -= num || 1;
      // 规格下标， 商品数量(多规格时可以选数量)
      // 存入购物车的数量不得大于20,核销的数量可能超过20 ，所以这里限制大于等于20
      if (this.alreadySelectedListsLen >= 20)
        return this.$message.error("开单商品数量不得超过20！");
      let goods = JSON.parse(JSON.stringify(this.product));
      // goods.stockId = this.product.specificationInfoList[idx].id; // 库存不足判定的时候用到的
      ...
      let params = {
        ...
        sendType: 0, // 物流配送? 服务没有,产品有 0-门店自提 1-物流配送
        stock: this.product.specificationInfoList[idx].stock, // 库存? 服务没有,产品有
        deleted: false, // 是否删除
        storeRemark: "", // 门店备注
        paymentStatus: false, // 未支付
        storePreference: null, // 门店优惠
        specificationId: this.product.specificationInfoList[idx].id, // 对应的规格的id
        shopPrice: this.product.specificationInfoList[idx].shopPrice, // 对应的规格的门店价， [需覆盖]产品的字段
        snapshotId: this.product.specificationInfoList[idx].snapshotId, // 对应的规格的快照id
        specificationName: this.product.specificationInfoList[idx].name, // 对应的规格的名称
        memberPrice: this.product.specificationInfoList[idx].memberPrice, // 对应的规格的会员价， [需覆盖]产品的字段
        // specificationTime: this.product.specificationInfoList[idx].serviceTime, // 规格服务时长 - 这个应该是不需要的
        enableCardDiscount: this.product.specificationInfoList[idx]
          .enableCardDiscount, // 是否允许卡打折???
        tempFinalPrice:
          this.product.specificationInfoList[idx].memberPrice * num, // 临时最终价 - 同规格会员价(这个数字是要乘数量的)
        staffId: this.billInfo.billingStaff.staffId, // 作为开单人
        staffName: this.billInfo.billingStaff.name, // 作为开单人
        assistantOrSalers: [] // 工位2， 销售人?
        // 产品不用工位
        // stationThree: [], // 工位3
        // stationOne: goods.operate ? [ // 工位1
        //   { // 开单人自动加入第一工位
        //     ratio: 100, // 业绩比例
        //     isChosen: false, // 非指定
      ...
      if (this.billInfo.customerInfo.id) {
        this.memberPriceCompareInfo(params, idx);
      } else {
        this.productAdd(params, idx)
      }
    ...
    memberPriceCompareInfo(params, idx) {
    ...
    // 产品叠加的方法
    productAdd(params, idx) {
      // 相同产品可以叠加
        let selectedGoods = this.billInfo.selectedGoods.filter(i => !i.deleted);
        let ids = selectedGoods.map(i => i.specificationId);
        if (
          ids &&
          ids.length &&
          ids.indexOf(this.product.specificationInfoList[idx].id) !== -1
        ) {
          this.billInfo.selectedGoods.some((i, index) => {
            if (
              i.specificationId == this.product.specificationInfoList[idx].id
            ) {
              params.count = i.count + 1;
              params.tempFinalPrice = params.memberPrice * params.count;
              this.setSelectedGoodsValue({ ...params, index });
            }
          });
        } else {
  ...
  computed: {
    ...
    SumStock() {
      let sum = 0;
      this.product.specificationInfoList.forEach(item => {
        sum += item.stock;
      });
      return sum;
    }
