        ...
        <td class="second-line-detail commission">
          <div class="staff-commission" @click="addAssistOrSalesman(detail,index)">
            <div class="assist-items">
              ...
              <span style="color:#C7CAD1" v-else>
      ...
      <tr>
        <td colspan="2" class="remark">
          <div class="remark-content">
            ...
            />
            <el-radio-group v-model="sendType" :disabled="detail.paymentStatus">
...
export default {
  ...
  mounted() {
    ...
    this.sendType = this.detail.sendType;
  },
  methods: {
    ...
    handleGetEditedWorkStations(data) {
      ...
      let currentKey = "assistantOrSalers";
    ...
    addAssistOrSalesman(item) {
      ...
      this.currentIndexStaffs = this.detail["assistantOrSalers"];
    }
  },
  computed: {
    ...mapState({
    ...
    commissionPercentage() {
      ...
      list.forEach(assists => {
        let shopNum = this.shopStaffsInfo[assists.id || assists.staffId]
          ? `(${this.shopStaffsInfo[assists.id || assists.staffId]})`
          : "";
        s +=
          assists.name +
          shopNum +
          " " +
          Math.round(assists.ratio * 100) / 100 +
          "%；";
      });
      return s;
    }
  ...
  watch: {
    ...
    sendType: {
      handler: function(v, o) {
        this.setSelectedGoodsValue({ index: this.index, sendType: v });
      },
      immediate: true
    },
