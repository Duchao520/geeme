<template>
  <div class="card-item-root" v-loading="dialogLoading">
    <!-- 新购时的身份卡和储值卡的显示 -->
    <div class="card-table-box" v-if="showInfo(0,[0,1]) || showInfo(2,[1])">
      ...
      <div class="box-item add" v-if="cardInfo.cardType===1">
        ...
        <p class="box-item-bottom">
          <number-input v-model="cardInfo.chargeMoney" @input="changePrepaidChargeMoney" :noBlur="true"></number-input>
        </p>
      </div>
      <div class="box-item give" v-if="cardInfo.cardType===1">
        ...
        <p class="box-item-bottom">
          <number-input v-model="cardInfo.giftMoney" @input="changePrepaidGiftMoney" :noBlur="true"></number-input>
        </p>
      </div>
    ...
    <div class="card-table-box" v-if="showInfo(1,[0,1])">
      ...
      <div class="box-item add">
        ...
        <p
          class="box-item-bottom"
        >{{cardInfo.validPermanent?'永久有效': cardInfo.extendTime || cardInfoShowTime}}</p>
      </div>
...
export default {
  ...
  computed: {
    ...
    // 取单回显期限延长字段 // 后端没有提供extendTime
    cardInfoShowTime() {
      let cardInfo = JSON.parse(JSON.stringify(this.cardInfo));
      if (cardInfo.validUnit == "month") {
        return `${cardInfo.validNum}个月`;
      } else if (cardInfo.validUnit == "year") {
        return `${cardInfo.validNum}年`;
      } else {
        return `${cardInfo.validNum}日`;
      }
    }
  },
  methods: {
    ...
    modifyCountCard(data) {
      ...
      // 这个字段留着会使次卡共用次数问题出现错误
      // delete data.countLimitStatus
      data.countLimit = data.totalCountLimit
      for (let key in data) {
    ...
    changePrepaidChargeMoney(num) {
      console.log(num)
      if (this.cardInfo.operateType == 1) return;
      // 变为自由卡 售价跟随变化
      this.setSelectedCardsValue({
        chargeMoney: num,
        operateType: 2,
