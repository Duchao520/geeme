<template>
  <el-dialog title="选择产品" :visible="visible" :before-close="handleClose">
    <div class="time-card-select-service">
      ...
      >
        ...
        <el-table-column label="产品名称" show-overflow-tooltip prop="goodsName"></el-table-column>
        <el-table-column label="规格" show-overflow-tooltip prop="specificationName"></el-table-column>
        <el-table-column label="专柜价" show-overflow-tooltip prop="shopPrice">
          <template slot-scope="scope">
            ￥{{scope.row.shopPrice}}
          </template>
        </el-table-column>
        <el-table-column label="库存" show-overflow-tooltip prop="stock"></el-table-column>
      </el-table>
...
export default {
  ...
  created() {
    let serviceData =  {goodsTypeScope: 'production', storeId: this.currentInfo.shopId}
  ...
  methods: {
    init() {
      ...
      let url = this.isGroupBuy ? '/getStoreGoodsListForGroupPurchase' : '/getStoreGoodsListForStore'
      params.currentPage--
      axios.post(
        url, 
