  ...
  <div class="bind-client">
    ...
    <!-- <div class="radio-group">
      <el-radio-group v-model="params.sellStatus" size="small">
        ...
        <el-radio-button :label="2">未成单</el-radio-button>
      </el-radio-group>
    </div> -->
    <!-- 表格主体 -->
    <div class="table-main">
      <el-table stripe :data="inviteSellerData" height="calc(100vh - 423px)" v-loading="loading">
        <el-table-column prop="businessName" label="推客信息">
        ...
        <el-table-column prop="createTime" label="成为推客时间">
          <template slot-scope="{row}">
            <p>{{utils.formatDateTimeFromYearToSecond(row.createTime)}}</p>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getBindSeller() {
      this.loading = true;
      const res = await axios.post("/getChildListBySellerId", {
        ...this.params,
        sellerId: this.sellerId
      });
      this.loading = false;
      this.total = res.total;
      this.inviteSellerData = res.sellerInfoList;
    },
    // 解绑
    unbundle(row) {
    ...
    async unbundleClient() {
      ...
      const res = await axios.post('/unbindChildSeller',{
        sellerId:this.currentClient.id
      })
      if(!res.success) return this.$message.error('解绑推客失败')
      this.$message.success('解绑推客成功')
