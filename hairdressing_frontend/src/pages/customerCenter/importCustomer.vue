    ...
    <div class="header-btn-line">
      <div class="left-btn">
        <el-upload
          ...
          :limit="1"
        >
          <el-button class="resize-btn normal_red_btn" :loading="btnLoad">导入客户资料</el-button>
        </el-upload>
        <el-upload
          class="upload-demo"
          accept=".xls"
        ...
        >
          ...
          <el-button class="resize-btn normal_green_btn" :loading="btnLoad">导入会员卡信息</el-button>
        </el-upload>
      </div>
      <div class="right-btn">
        <div class="customer-template">
          <i
            ...
            @click="downloadTemplate('customer')"
          >客户资料模板</i>
        </div>
      ...
      >
        <el-table-column label="类型">
          <template slot-scope="scope">
            <span>{{scope.row.type === 1 ? '客户导入记录' : '会员卡导入记录'}}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    downloadHistory(obj) {
      // 下载当前历史文件
      axios
        .get(`/downloadImportResult/${obj.resultFileKey}`, {
          responseType: "blob"
        })
        .then(res => {
          let a = document.createElement("a");
          ...
          a.download =
            obj.type === 1 ? "客户导入结果.xls" : "会员卡导入结果.xls";
          a.click();
          window.URL.revokeObjectURL(url);
        });
    },
