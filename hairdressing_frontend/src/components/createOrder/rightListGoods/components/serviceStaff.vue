<template>
  <div class="service-staffs-box" :class="{single: total === 1}">
    ...
    <div class="single-staff-box" v-else @click="setStationStaff">
      <!-- 只显示两个, 切换状态去弹窗里处理 -->
      <div class="single-staff" v-for="staff in listStaff" :key="staff.id">
        <!-- 店内编号还没有 -->
        ...
        <p>
          <span class="send-type-tags text-cant-copy" :class="{active: staff.isChosen}" @click.stop="staff.isChosen = !staff.isChosen">{{staff.isChosen ? '指 定' : '非指定'}}</span>
