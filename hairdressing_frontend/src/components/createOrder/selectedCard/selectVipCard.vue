...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAvailableVipCards(showSelectIdentityCardModel) {
        ...
        let params = {
          ...
          listCountProductInfo: availableGoodsList
            .filter(item => item.goodsType === "production")
            .map(item => {
              return {
                amount: item.num,
                productSpecificationsId: item.specificationId,
                productSpecificationsSnapshotId: item.snapshotId,
                storePreference: item.storePreference
              };
            }),
          listCountServiceInfo: availableGoodsList
            .filter(item => item.goodsType === "service")
            .map(item => {
              return {
                amount: item.num,
                serviceSpecificationsId: item.specificationId,
                serviceSpecificationsSnapshotId: item.snapshotId,
                storePreference: item.storePreference
              };
            })
        };
        axios.post("/getAllCardPayGoodsCountInfo", params).then(res => {
          /**
           * 打开弹窗：点击选择卡片的那次对卡的请求
           * 不打开弹窗：增减商品的请求
           * */
          this.selectIdentityCardVisible = showSelectIdentityCardModel;
          ...
          let cardList = (res.listCardPayGoodsCountInfo || [])
            ...
            .concat(
              (res.listCountCardGoodsCountInfo || []).map(item => {
                return {...item, type: "countCard"};
              })
            )
            .concat(
              (res.listTimeCardGoodsCountInfo || []).map(item => {
                return {...item, type: "timeCard"};
              })
            )
            .concat(
              (res.listIdentityGoodsCountInfo || []).map(item => {
                return {...item, type: "identity", canPay: true};
              })
            );
