...
export default {
  ...
  created () {
    this.serverUrl = api.uploadPicture.URL
    // console.log(utils.getCookieToken('geeme_pc_token'))
    let token = utils.getCookieToken('geeme_pc_token')
    // let headers = isAuth ? { Authorization: (!!token && token[2].length) ? token[2] : '' } : {}
    this.header.Authorization = (!!token && token[2].length) ? token[2] : ''
  },
  methods: {
    onEditorBlur() {
      //失去焦点事件
    ...
    uploadSuccess(res, file) {
      ...
      if (res.success == true) {
        ...
        quill.insertEmbed(length, "image", IMAGE_URL + '/' + res.fileKey +'!circle160');
        // 调整光标到最后
