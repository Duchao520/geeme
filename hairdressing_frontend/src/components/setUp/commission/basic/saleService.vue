<template>
  <div class="service-table">
    ...
    >
      ...
      <el-table-column label="扣成本" prop="performanceCost" width="160px">
        <template slot-scope="scope">
          <!-- <el-input max='999999' v-model.number="scope.row.coperformanceCostst" @change="changeSingleRow(scope.row)" size="medium">
            <template slot="append">元</template>
          </el-input> -->
          <div class="cost-input-box">
            <input class="cost-input-before" maxlength="12" v-model.number="scope.row.performanceCost"  @keyup="changeSingleRow(scope.row)">
...
export default {
  ...
  methods: {
    ...
    saveData() {
      ...
      this.tableData.map(item => {
        params.push({
          ...
          productCostType: 'SERVICE', // SERVICE, PRODUCT, CARD
          productId: item.serviceId,
          cost: item.performanceCost,
    ...
    // 输入框输入事件
    changeSingleRow(row) {
      if (/^[0-9]{1,12}(\.[0-9]{0,2})?$/.test(row.performanceCost + '')) {
        if (this.checkedIds.includes(row.specificationId)) {
          this.multipleSelection.map(i => {
            i.performanceCost = row.performanceCost
          })
        }
      } else {
