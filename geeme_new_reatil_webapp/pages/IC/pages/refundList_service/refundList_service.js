...
const app = getApp()
Page({
  $name: 'refundList_service',
  data: {
    tabList: ['全部', '退款中', '退款完成'],
    orderStatusArr: ['ALL', 'REFUNDING', 'REFUNDED'],
    // request相关参数
    orderStatus: 'ALL',
    pageNum: 0,
    pageSize: 10,
  ...
  getRefundList() {
    ...
    wx.showLoading()
    apiAction.getServiceRefundOrderList(params).then(data => {
      wx.hideLoading()
  ...
  cancelOrder(e) {
    ...
    wx.showModal({
      ...
      success: res => {
        if (res.confirm) {
          wx.showLoading()
          apiAction.cancelServiceRefundOrder(id).then(data => {
            wx.hideLoading()
            if (data.success) {
