 ...
 Page({
   data: {
    ...
    params: {
     ...
     total: null,             //明细总数
     switcher: true,         //请求控制器
     suitStoreInfoForPagedList: [],         //权益金门店列表
     current: 0,
     /* <充值组件中的数据> */
     ...
     isShowStoreList: false    // 显示可用门店
   },
   computed: {
     currentCard() {
       if (this.data.list && this.data.current + 1 <= this.data.list.length) {
         this.data.params = {
           cardKindId: this.data.list[this.data.current].cardKindId,
           cardCustomerId: this.data.list[this.data.current].cardCustomerId,
           currentPage: 0,
           pageSize: 10,
           paged: true
         }
  ...
  //  指定会员卡门店
   onTapshowStore() {
     this.showStoreList('firstPage')
   },
   showStoreList(next) {
     let { params, total, switcher, suitStoreInfoForPagedList } = this.data;
     if (!switcher) return;
     this.setData({
       switcher: false
     })
     if (next == 'firstPage') {
       ...
       if (total <= suitStoreInfoForPagedList.length) return;
       params.currentPage++;
     }
     return apiAction.getStoreCardList(params).then(res => {
       suitStoreInfoForPagedList = suitStoreInfoForPagedList.concat(res.suitStoreInfoForPagedList)
       this.setData({
         suitStoreInfoForPagedList: suitStoreInfoForPagedList,
         total: res.total,
  ...
  //  监听弹框滚动到底部
   ...
   onTapCardSystem(e) {
     let params = {
       cardDetail: JSON.stringify(e.currentTarget.dataset.item),
       currentCard: JSON.stringify(this.data.currentCard)
     }
     this.$route('../VipCardManagement/VipCardManagement', params)
   },
   /**
    * 跳转会员卡详情
    */
