        ...
        methods:{
          getErwei(){
            ...
            getSignIn(_data).then(sec=>{
               Tips.loading('生成中');
              _this.$http.useCoupon(_data,sec).then(res=>{
                console.log('请求回来的参数',res);
          ...
          getScanData(){
            var _data ={
              token:this.$store.state.token,
              couponBarCode:this.qrNumber,
            }
            var _this = this;
            getSignIn(_data).then(sec=>{
              _this.$http.couponStatu(_data,sec).then(res=>{
                ...
                if(res.data.state==2){
                  ...
                  if(par.payChannelStoreNum) {
                    wx.redirectTo({
                      url: `../../pages/couponPay/main?orderNum=${par.orderNum}&totalMoney=${par.totalMoney}&shouldPayMoney=${par.shouldPayMoney}&couponMoney=${par.couponMoney}&integralCount=${par.integralCount}&consumeTime=${par.consumeTime}&storeName=${par.storeName}&storeId=${par.storeId}&activeNum=${par.activeNum}&mercId=${par.payChannelStoreNum}&trmNo=${par.trmNo}&id=${_this.id}`
                    })
                  }else{
                    Tips.toast('该商家未开启支付通道');
                  }
                }
              })
            })
