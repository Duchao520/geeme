...
import Tips from '@/common/js/tips'
  export default {
    ...
    mounted() {
       ...
       this.integralConvertCash = par.integralConvertCash;
        ...
        if (_self.payState === '') {
          ...
          if ( _self.payState == 1) {
            var _data = {
              ...
              integralCount:_self.integralConvertCash,
              subject:'用户扫码支付',
              wxUnionId: _self.$store.state.unionid ? _self.$store.state.unionid : '',
              isOriginPricePay: _self.activeNum ? '' : 1
            }
            _self.$http.unifiedpay(_data).then(res=>{
              if(res.state == 1){
                wx.requestPayment({
                  ...
                  success:function(recc){
                    ...
                    wx.redirectTo({
                      url:`/pages/packageB/pages/paySuc/main?orderNum=${res.orderNum}&shopStoreId=${_self.storeId}`
                    })
                  },
                  fail:function(res){
                    console.log('失败',res);
