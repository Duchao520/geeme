<template>
  <div class="index" @click="isShowKeyboard(0); type = 0">
    <div class="shopInfo flex_box box_padding" style="padding: 38rpx 46rpx 0;">
      ...
      <div class="flex_box flex_column justify_space_between" style="width: 250rpx;height: 90rpx;">
    ...
    <div class="module">
      <div class="pay card" @click="priType = priType == 1 ? '' : 1" v-if="svipIsShow">
      ...
      <div class="pay flex-box" @click="priType = priType == 2 ? '' : 2" v-if="actIsShow">
    ...
    <footer>
      <div class="vipDiscount flex_box" v-if="!allienceDiscountCard.userRole && priType == 3">
        ...
        <p class="fz-24 fc-ffffff">升级VIP优惠{{ALLIENCE_PAY.levelUpDiscount}}元</p>
      </div>
      <div class="allienceDiscount flex_box justify_space_between align_center box_padding" style="padding: 0 24rpx;"
           v-if="priType == 3">
        <p class="fz-24" style="color: #80521C">您使用助商通联盟打折卡优惠<span class="fc-F7094D">{{ALLIENCE_PAY.allienceDiscount_ceil}}元</span></p>
        <p class="fz-24 fc-F7094D">查看联盟打折卡</p>
      </div>
    ...
    methods: {
      ...
      zstPay() {
        ...
        var _data = {
          ...
          integralCount:String(_self.accDiv(_self.discount, 100)),
          totalmoney:_self.user.totalmoney?_self.user.totalmoney:'',
        };
        _self.submitScanPay(_data);
      },

      submitScanPay(_data){
        var _self = this;
        getSignIn(_data).then(sec => {
          _self.$http.scanPay(_data,sec).then(res => {
            if (res.state == 1) {
              if(_data.amount==0){
        ...
        getSignIn(_data).then(sec => {
          _self.$http.getPayDetail(_data, sec).then(res => {
            if (res.state == 1) {
              ...
              _self.cardDetail.shareCardDiscount_ceil = _self.accMul(Number(_self.cardDetail.shareCardDiscount),10);
              _self.shopId = res.data.shopId;
              ...
              if (_self.shopPayChannelCount && _self.shareCardCount) {             //有共享卡，开通收银
                ...
                _self.allienceDiscountCard ? _self.DEFAULT_PRITYPE = 3 : _self.DEFAULT_PRITYPE = 1;
                _self.priType = 1;
              } else if (_self.shopPayChannelCount && !_self.shareCardCount) {       //没有有效的共享卡 ，开通收银
                _self.actIsShow = true;
                _self.couponIsShow = true;
                _self.allienceDiscountCard ? _self.DEFAULT_PRITYPE = 3 : _self.DEFAULT_PRITYPE = 2;
                _self.priType = 2;
      ...
      submitSvipPay(_data) {
        var _self = this;
        getSignIn(_data).then(sec => {
          _self.$http.wxPay(_data, sec).then(res => {
            if (res.state == 1) {
              if(_data.amount == 0){
                Tips.toast(`付款成功`, 'none');
                wx.navigateTo({
                ...
                if(res.data.resultCode != -1) {
                  wx.requestPayment({
                    ...
                    success: function (cc) {
                      Tips.toast(`付款成功`, 'none');
                      wx.navigateTo({
                        url: `../../pages/paySuc/main?orderNum=${res.orderNum}&shopStoreId=${_self.storeId}&shareCardId=${_self.cardDetail.id}`
                      })
                    },
                    fail: function (res) {
                      if (res.errMsg == 'requestPayment:fail cancel') {
      ...
      originalPayPublic() {
        var _self = this;
        _self.actPay = Number(_self.accMul(_self.totalMoney, 100));
        _self.zstPay();
      },
        ...
        if(_self.coupon && Number(_self.Subtr(Number(_self.oneLevelMoney), _self.coupon.couponMoney)) <0 && (Number(_self.oneLevelMoney) >= _self.coupon.consumeMoney)){   //实付金额小于卡券优惠
          ...
          if(!_self.$store.state.shareCardId){
            if(_self.mapCard()){
              ...
              _self.svipActPay = _self.accMul(_self.disMoney,100);
              _self.svipDiscount = 0;
            }
          }else{
            _self.svipActPay = Math.ceil(_self.accAdd(_self.accMul(_self.accMul(_self.disMoney, _self.cardDetail.shareCardDiscount ? _self.cardDetail.shareCardDiscount : 0), 100), _self.accMul(_self.notDisMoney, 100)));
          ...
          //联盟打折卡
          var ALLIENCE_VIP_ACTPAY = _self.accAdd(_self.accMul(_self.disMoney,_self.accMul(Number(_self.allienceDiscountCard.vipShareDiscountCardRate),10)),_self.accMul(_self.notDisMoney, 100));
          var ALLIENCE_USER_ACTPAY = _self.accAdd(_self.accMul(_self.disMoney,_self.accMul(Number(_self.allienceDiscountCard.comShareDiscountCardRate),10)),_self.accMul(_self.notDisMoney, 100));
          ...
          _self.ALLIENCE_PAY.levelUpDiscount = _self.accDiv(_self.Subtr(ALLIENCE_USER_DISCOUNT,ALLIENCE_VIP_DISCOUNT),100);
