...
Page({
  ...
  onLoad(options) {
    ...
    app.getBrand((brand) => {
      ...
      util.PromiseAll([
        this.getProductList(), this.getTagsInfo()
      ]).then(res => {
  ...
  search(e){
    ...
    this.initPage()
    return this.getProductList().then(res => {this.$setData(res); return res});
  },
  selectClass(e){
  ...
  getClassInfo(){
    return apiAction.weGetStoreClassInfoInUsing('production').then( data => {
  ...
  // 获取产品列表
  getProductList() {
    const { pageNum, pageSize, goodsOrderByEnum, keyword, productLevelOne, productLevelTwo, tagId,brandId} = this.data
    let params = { 
      pageNum: pageNum,
      pageSize: pageSize,
      goodsOrderByEnum,
    ...
    wx.showLoading()
    return apiAction.weGetBrandProductListByCondition(params).then(data => {
      wx.hideLoading()
      if (!data.productInfos.length) {
        return {};
      }
      let list = this.data.pageNum == 0 ? data.productInfos : this.data.goodsList.concat(data.productInfos)
  ...
  getTagsInfo(){
    return apiAction.weGetStoreGoodsTagsInUsing('production').then( data => {
